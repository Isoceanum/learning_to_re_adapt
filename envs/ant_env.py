...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def get_model_reward_fn(self):
        ...
        qpos_dim = self.observation_structure["qpos"]
        qvel_dim = self.observation_structure["qvel"]
        cfrc_dim = self.observation_structure["cfrc_ext"]
        
        vel_start = qpos_dim
        vel_end = qpos_dim + qvel_dim
        cfrc_start = vel_end
        cfrc_end = vel_end + cfrc_dim
        ...
        def reward_fn(state, action, next_state):
            ...
            # Slice qpos (positions)
            pos = state[:, :qpos_dim]
            pos_next = next_state[:, :qpos_dim]

            # Slice qvel (velocities)
            vel = state[:, vel_start:vel_end]
            vel_next = next_state[:, vel_start:vel_end]

            # Slice external contact forces (if present)
            if cfrc_dim > 0:
                cfrc = state[:, cfrc_start:cfrc_end]
                cfrc_next = next_state[:, cfrc_start:cfrc_end]
            else:
                cfrc = None
                cfrc_next = None
            ...
            # Control cost
            ctrl_cost = self._ctrl_cost_weight * torch.sum(action ** 2, dim=-1)
            
            
            # Contact cost
            if cfrc_dim > 0:
                contact_cost = self._contact_cost_weight * torch.sum(cfrc_next ** 2, dim=-1)
            else:
                contact_cost = 0.0
                
            # Healthy term (approximate)
            min_z, max_z = self._healthy_z_range
            z_next = pos_next[:, 2]

            finite_mask = torch.isfinite(next_state).all(dim=-1)
            healthy_mask = finite_mask & (z_next >= min_z) & (z_next <= max_z)
            healthy_term = self._healthy_reward * healthy_mask.float()
            
            total_reward = forward_reward + healthy_term - ctrl_cost - contact_cost
            return total_reward
