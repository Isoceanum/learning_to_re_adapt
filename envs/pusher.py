...
XML_PATH = str(Path(__file__).with_name("assets") / "pusher.xml")
...
class PusherEnv(MujocoEnv, utils.EzPickle):
   ...
   # Expose a model-based reward function for planning (Torch)
    def get_model_reward_fn(self):
        import torch

        # Cache weights as torch tensors (moved to correct device inside reward_fn)
        w_near = torch.tensor(self._reward_near_weight, dtype=torch.float32)
        w_dist = torch.tensor(self._reward_dist_weight, dtype=torch.float32)
        w_ctrl = torch.tensor(self._reward_control_weight, dtype=torch.float32)

        # Indices must match _get_obs() in THIS codebase (23D obs):
        # [0:7]   qpos[:7]
        # [7:14]  qvel[:7]
        # [14:17] tips_arm (x,y,z)
        # [17:20] object   (x,y,z)
        # [20:23] goal     (x,y,z)
        TIP_SL = slice(14, 17)
        OBJ_SL = slice(17, 20)
        ...
        def reward_fn(state, action, next_state):
            # We don't actually need `state` for this env's reward,
            # but keep it in the signature for consistency.
            assert torch.is_tensor(state) and torch.is_tensor(action) and torch.is_tensor(next_state)
            assert next_state.shape[-1] >= 23, f"Expected next_state[..., 23], got {next_state.shape}"

            device = next_state.device
            w_near_d = w_near.to(device)
            w_dist_d = w_dist.to(device)
            w_ctrl_d = w_ctrl.to(device)

            tip = next_state[..., TIP_SL]     # (..., 3)
            obj = next_state[..., OBJ_SL]     # (..., 3)
            goal = next_state[..., GOAL_SL]   # (..., 3)

            vec_1 = obj - tip                 # object - fingertip
            vec_2 = obj - goal                # object - goal

            reward_near = -torch.norm(vec_1, p=2, dim=-1) * w_near_d
            reward_dist = -torch.norm(vec_2, p=2, dim=-1) * w_dist_d
            reward_ctrl = -torch.sum(action * action, dim=-1) * w_ctrl_d
