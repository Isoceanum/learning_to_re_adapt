...
class HalfCheetahEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release.
    ...
    def step(self, action):
        ...
        reward, reward_info = self._get_rew(x_velocity, rooty, action)
        info = {"x_position": x_position_after, "x_velocity": x_velocity, **reward_info}

        terminated = abs(float(rooty)) > 3.0
    ...
    def _get_rew(self, x_velocity: float, rooty: float, action):
        forward_reward = self._forward_reward_weight * x_velocity
        ctrl_cost = self.control_cost(action)

        ay = abs(float(rooty))

        # progressive penalty: 0 below 1.5, ramps up to max at 3.0
        penalty = 0.0
        if ay > 1.5:
            t = min((ay - 1.5) / 1.5, 1.0)   # maps [1.5, 3.0] -> [0, 1]
            penalty = 20.0 * (t * t)         # scale=20, quadratic ramp

        # if totally flipped, don't allow forward reward
        if ay > 3.0:
            forward_reward = 0.0

        reward = forward_reward - ctrl_cost - penalty
    ...
    def get_model_reward_fn(self):
        ...
        def reward_fn(state, action, next_state):
            ...
            ay = torch.abs(state[..., 2])

            # same progressive penalty as env
            t = torch.clamp((ay - 1.5) / 1.5, 0.0, 1.0)
            penalty = 50.0 * (t * t)

            # zero forward reward if flipped beyond 3.0
            forward_reward = torch.where(ay > 3.0, torch.zeros_like(forward_reward), forward_reward)

            ctrl_cost = ctrl_cost_weight.to(state.device) * torch.sum(action * action, dim=-1)
            return forward_reward - ctrl_cost - penalty
