...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        forward_reward_weight: float = 1.8,
        ctrl_cost_weight: float = 0.4,
        contact_cost_weight: float = 5e-4,
        healthy_reward: float = 0.7,
        target_forward_speed: float = 1.5,
        overspeed_cost_weight: float = 0.15,
        backward_cost_weight: float = 0.5,
        healthy_progress_gate_min: float = 0.2,
        main_body: int | str = 1,
        ...
        **kwargs,
        ...
        utils.EzPickle.__init__(
            ...
            overspeed_cost_weight,
            backward_cost_weight,
            healthy_progress_gate_min,
            main_body,
        ...
        self._target_forward_speed = target_forward_speed
        self._overspeed_cost_weight = overspeed_cost_weight
        self._backward_cost_weight = backward_cost_weight
        self._healthy_progress_gate_min = healthy_progress_gate_min
    ...
    def step(self, action):
        ...
        reward, reward_info = self._get_rew(obs_before, action, observation)
    ...
    def _get_rew(self, state, action, next_state):
        x_velocity = next_state[..., 13]
        forward_velocity = np.clip(x_velocity, 0.0, self._target_forward_speed)
        forward_reward = forward_velocity * self._forward_reward_weight
        backward_speed = np.maximum(-x_velocity, 0.0)
        overspeed = np.maximum(x_velocity - self._target_forward_speed, 0.0)
        backward_cost = self._backward_cost_weight * (backward_speed * backward_speed)
        overspeed_cost = self._overspeed_cost_weight * (overspeed * overspeed)

        ctrl_cost = self.control_cost(action)

        z = next_state[..., 0]
        min_z, max_z = self._healthy_z_range
        finite = np.isfinite(state).all() and np.isfinite(next_state).all()
        healthy = finite and (min_z <= z <= max_z)
        speed_gate = np.clip(
            x_velocity / max(self._target_forward_speed, 1e-6),
            0.0,
            1.0,
        )
        healthy_scale = self._healthy_progress_gate_min + (1.0 - self._healthy_progress_gate_min) * speed_gate
        healthy_reward = (self._healthy_reward * healthy_scale if healthy else 0.0)

        quat_x = next_state[..., 2]
        quat_y = next_state[..., 3]
        tilt_cost = 0.10 * (quat_x * quat_x + quat_y * quat_y)

        y_velocity = next_state[..., 14]
        lateral_cost = 0.05 * (y_velocity * y_velocity)

        ang_vel = next_state[..., 16:19]
        ang_cost = 0.01 * float(np.sum(ang_vel * ang_vel))
        ...
        reward_info = {
            ...
            "reward_tilt": -tilt_cost,
            "reward_lateral": -lateral_cost,
            "reward_ang": -ang_cost,
            "reward_backward": -backward_cost,
            "reward_overspeed": -overspeed_cost,
        }
    ...
    def get_model_reward_fn(self):
        ...
        target_forward_speed = torch.tensor(self._target_forward_speed, dtype=torch.float32)
        overspeed_cost_weight = torch.tensor(self._overspeed_cost_weight, dtype=torch.float32)
        backward_cost_weight = torch.tensor(self._backward_cost_weight, dtype=torch.float32)
        healthy_progress_gate_min = torch.tensor(self._healthy_progress_gate_min, dtype=torch.float32)

        min_z = torch.tensor(self._healthy_z_range[0], dtype=torch.float32)
        max_z = torch.tensor(self._healthy_z_range[1], dtype=torch.float32)

        tilt_cost_weight = torch.tensor(0.10, dtype=torch.float32)
        lateral_cost_weight = torch.tensor(0.05, dtype=torch.float32)
        angvel_cost_weight = torch.tensor(0.01, dtype=torch.float32)
        ...
        def reward_fn(state, action, next_state):
            ...
            v_target = target_forward_speed.to(device)
            w_overspeed = overspeed_cost_weight.to(device)
            w_backward = backward_cost_weight.to(device)
            healthy_gate_min = healthy_progress_gate_min.to(device)
            z_min = min_z.to(device)
            z_max = max_z.to(device)
            w_tilt = tilt_cost_weight.to(device)
            w_lat = lateral_cost_weight.to(device)
            w_ang = angvel_cost_weight.to(device)
            eps = torch.tensor(1e-6, dtype=state.dtype, device=device)

            x_velocity = next_state[..., 13]
            forward_velocity = torch.minimum(torch.clamp(x_velocity, min=0.0), v_target)
            forward_reward = w_fwd * forward_velocity
            backward_speed = torch.clamp(-x_velocity, min=0.0)
            backward_cost = w_backward * (backward_speed * backward_speed)
            overspeed = torch.clamp(x_velocity - v_target, min=0.0)
            overspeed_cost = w_overspeed * (overspeed * overspeed)

            ctrl_cost = w_ctrl * torch.sum(action * action, dim=-1)

            z = next_state[..., 0]
            finite = torch.isfinite(state).all(dim=-1) & torch.isfinite(next_state).all(dim=-1)
            in_range = (z >= z_min) & (z <= z_max)
            healthy = finite & in_range
            speed_gate = torch.clamp(x_velocity / torch.maximum(v_target, eps), min=0.0, max=1.0)
            healthy_scale = healthy_gate_min + (1.0 - healthy_gate_min) * speed_gate
            healthy_reward = h_rew * healthy_scale * healthy.to(state.dtype)

            quat_x = next_state[..., 2]
            quat_y = next_state[..., 3]
            tilt_cost = w_tilt * (quat_x * quat_x + quat_y * quat_y)

            y_velocity = next_state[..., 14]
            lateral_cost = w_lat * (y_velocity * y_velocity)

            ang_vel = next_state[..., 16:19]
            ang_cost = w_ang * torch.sum(ang_vel * ang_vel, dim=-1)
