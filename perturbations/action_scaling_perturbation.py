...
class ActionScalingPerturbation(Perturbation):
    """Scales specific action dimensions by a factor or within a range."""
    ...
    def reset(self, env):
        super().reset(env)
        if self.active:
            self.current_index = int(np.random.choice(self.effected_action_indices))
            low, high = self.scale_range
            self.current_scale = np.random.uniform(low, high)
        else:
            self.current_index = None
            self.current_scale = 1.0
    ...
    def apply_action(self, action):
        if not self.active or self.current_index is None:
            return action
        action = action.copy()
        action[self.current_index] *= self.current_scale
        return action
