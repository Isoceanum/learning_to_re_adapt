...
class TaskSpecificAllParameterAdaptationTrainer(BaseTrainer):
    def __init__(self, config, output_dir):
        ...
        self._enable_full_training(self.dynamics_model)
        self.optimizer = self._make_full_optimizer()
    ...
    def _enable_full_training(self, model):
        for param in model.parameters():
            param.requires_grad_(True)
        model.train()

    def _make_full_optimizer(self):
        lr = float(self.train_config.get("learning_rate"))
        return torch.optim.Adam(self.dynamics_model.parameters(), lr=lr)
    ...
    def train(self):
        print("Starting TS-AllParams offline training")
    ...
    def save(self):
        save_path = os.path.join(self.output_dir, "all_params_model.pt")
        payload = {
            "all_params_model_state": self.dynamics_model.state_dict(),
        }
        if self.optimizer is not None:
            payload["optimizer_state"] = self.optimizer.state_dict()
        torch.save(payload, save_path)
        print(f"All-params model saved to {save_path}")

    def evaluate(self):
        print("Per-dim RMSE on eval split (base vs all-params)")
    ...
    def load(self, path):
        ...
        if os.path.isdir(checkpoint_path):
            checkpoint_path = os.path.join(checkpoint_path, "all_params_model.pt")
        if not os.path.exists(checkpoint_path):
            raise FileNotFoundError(f"No all-params checkpoint found at {checkpoint_path}")

        # Load base model (pretrained, frozen)
        base_model_path = self.train_config["pretrained_dynamics_model"]["model_path"]
        config_path = self.train_config["pretrained_dynamics_model"]["config_path"]
        obs_dim = self.env.observation_space.shape[0]
        action_dim = self.env.action_space.shape[0]
        self.dynamics_model = load_pretrained_dynamics_model(
            config_path, base_model_path, obs_dim, action_dim, self.train_seed, self.device
        )

        checkpoint = torch.load(checkpoint_path, map_location=self.device)
        state_dict = checkpoint.get("all_params_model_state", checkpoint)
        ...
        print(f"Loaded all-params model from {checkpoint_path}")
        return self
