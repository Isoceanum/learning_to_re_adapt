# TODO(meta_residual):
# - Replace this single-model implementation with a base dynamics model + residual adapter setup.
# - Base model should predict normalized delta (same as current), trained from scratch.
# - Add a ResidualAdapter module that predicts a correction in normalized delta space
#   (preferred to align with the base model and existing delta targets).
# - Build a wrapper that combines base + residual to produce next-state predictions.
# - Inner adaptation must update ONLY residual adapter parameters (full MAML, create_graph=True).
# - Outer/meta gradients must flow through the inner step and update BOTH base and residual params.
# - Provide parameter dict handling for functional_call across both modules.
# - Update compute_loss_with_parameters to accept base params + residual params and compute
#   loss on normalized delta (or normalized next-state if you explicitly choose that variant).
import torch
