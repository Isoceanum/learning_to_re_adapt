...
class TransitionBuffer:
    ...
    def sample_transitions(self, batch_size, support_window_size, query_window_size, split):
        # get request split 
        observations, actions, next_observations = self._get_split(split)
        
        # ensure we have data to sample
        if len(observations) == 0: 
            raise RuntimeError(f"No episodes available for split='{split}'")
        
        # total window length (support + query)
        window_size = support_window_size + query_window_size 
        
        # store sampled support (past) windows
        support_observations = []
        support_actions = []
        support_next_observations = []
        
        # store sampled query (future) windows
        query_observations = []
        query_actions = []
        ...
        # keep sampling until we have batch_size valid windows
        while len(support_observations) < batch_size:
            tries += 1
            # safety guard to avoid infinite loop in case episodes are too short
            if tries > max_tries: 
                raise RuntimeError(f"Could not sample {batch_size} windows of length {window_size} after {max_tries} attempts.")
            
            # sample random episode index from buffer
            episode_index = int(self.rng.integers(0, len(observations)))
            episode_length = len(observations[episode_index])
            
            # skip episodes if it doesn not contain at least window_size transitions
            if episode_length < window_size: 
                continue
            
            # sample a random start and end index for a full window
            start_index = int(self.rng.integers(0, episode_length - window_size + 1))
            end_index = start_index + window_size
            
            # slice window from this episode
            ...
            # split window into support (past M) and query (future K)
        ...
        # stack sampled windows into batched tensors
