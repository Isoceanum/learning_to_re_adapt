...
class DynamicsModel(nn.Module):
    ...
    def _assert_normalization_stats(self):
        if (
            self.mean_obs is None
            or self.std_obs is None
            or self.mean_act is None
            or self.std_act is None
            or self.mean_delta is None
            or self.std_delta is None
        ):
    ...
    def load_saved_model(self, model_path):
        ...
        if "state_dict" not in payload:
            raise KeyError(f"Checkpoint missing 'state_dict' in {model_path}")
        if "norm_stats" not in payload:
            raise KeyError(f"Checkpoint missing 'norm_stats' in {model_path}")
