...
class DynamicsModel(nn.Module):
    ...
    def compute_adapted_parameters_step(self, parameters, support_observations, support_actions, support_next_observations, inner_lr, create_graph):
      ...
      loss = self.compute_loss_with_parameters(support_observations, support_actions, support_delta, parameters)
      # compute gradients  for the support loss
      gradients = torch.autograd.grad(loss, tuple(parameters.values()), create_graph=create_graph)
      # apply one gradient step to get adapted parametersn
      adapted_parameters = OrderedDict((name, (param - inner_lr * grad)) for (name, param), grad in zip(parameters.items(), gradients))
    ...
    def compute_adapted_parameters(self, support_observations, support_actions, support_next_observations, inner_lr, create_graph):
        ...
        gradients = torch.autograd.grad(loss, tuple(base_parameters.values()), create_graph=create_graph)
