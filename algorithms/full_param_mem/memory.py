...
class FullParamMemory:
    ...
    def retrieve_with_temporary_adaptation(self, recent_transitions, dynamics_model,  support_window_size, inner_learning_rate):
        ...
        prior_loss = self._score_candidate(
            initial_params=theta_parameters,
        ...
        for entry in self.entries:
            ...
            candidate_loss = self._score_candidate(
                initial_params=candidate_parameters,
                dynamics_model=dynamics_model,
                obs_all=obs_all,
                act_all=act_all,
                next_obs_all=next_obs_all,
    ...
    def _score_candidate(self, initial_params, dynamics_model, obs_all, act_all, next_obs_all, support_window_size, inner_learning_rate, device, num_steps):
        ...
        for t in range(support_window_size, num_steps):
            ...
            adapted_params = dynamics_model.compute_adapted_parameters_step(
                candidate_params,
                support_obs,
                support_act,
                support_next_obs,
            ...
            with torch.no_grad():
                query_loss = dynamics_model.compute_loss_with_parameters(
                    query_obs, query_act, query_delta, adapted_params
                )
