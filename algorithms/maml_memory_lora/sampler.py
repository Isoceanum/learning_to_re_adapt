...
def sample_meta_batch(buffer, split, meta_batch_size, support_window_size, query_window_size, device):
    ...
    while len(support_observations) < meta_batch_size:
        ...
        # contiguous split from start of episode
        window_obs = observations[episode_index][:window_size]
        window_act = actions[episode_index][:window_size]
        window_next_obs = next_observations[episode_index][:window_size]
